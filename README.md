...
To adopt a Legacy artifact, use the authenticated inventory action to issue a short-lived one-time adoption code, then call the plugin's `artifact_adopt_legacy` tool with the Legacy ID, code, and a local slug. The plugin stores only a digest-keyed checkpoint, redeems the authorized current Revision, preserves its exact source bytes, and finalizes a local v1. The original Legacy artifact remains separate and read-only. Later `artifact_sync` creates a new cloud identity with the Legacy provenance attached.
