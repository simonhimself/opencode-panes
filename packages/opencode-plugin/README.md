...
OpenCode plugin that registers the legacy cloud `artifact` tool and the local-first `artifact_prepare`, `artifact_import`, and `artifact_finalize` tools for OpenCode Panes artifacts. This MIT-licensed workspace package is intentionally private and local-only.
